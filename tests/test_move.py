...
class TestMove(unittest.TestCase):
    ...
    def test_is_playable_information_color(self):
        mock_card = util.create_mock_card(color='blue')
        mock_gamestate = util.create_mock_gamestate(hands=[[mock_card, mock_card],
                                                           ['is_playable should', 'not look at players cards']])
        mock_gamestate.board = util.create_mock_board(clock_tokens=1)
    ...
    def test_is_playable_information_known(self):
        mock_card = util.create_mock_yourcard(public=True, number=2, color='blue')
        mock_gamestate = util.create_mock_gamestate(hands=[[mock_card, mock_card],
                                                           ['is_playable should', 'not look at players cards']])
        mock_gamestate.board = util.create_mock_board(clock_tokens=1)
    ...
    def test_is_playable_information_color_subset_of_cards(self):
        blue_card = util.create_mock_card(color='blue')
        red_card = util.create_mock_card(color='red')
        mock_gamestate = util.create_mock_gamestate(hands=[[blue_card, red_card, red_card, red_card],
                                                           ['is_playable should', 'not look at players cards']])
        mock_gamestate.board = util.create_mock_board(clock_tokens=1)
        info_dict = {'player_id': 0, 'information_type': 'color', 'information': 'blue'}
        move = Move(move_type='give_information', player_index=1, information=info_dict)
    ...
    def test_is_playable_information_number(self):
        mock_card = util.create_mock_card(number=2)
        mock_gamestate = util.create_mock_gamestate(hands=[['is_playable should', 'not look at players cards'],
    ...
    def test_is_playable_information_number_subset_of_cards(self):
        mock_card = util.create_mock_card(number=2)
        mock_card_not_two = util.create_mock_card(number=4)
        mock_gamestate = util.create_mock_gamestate(
            hands=[['is_playable should', 'not look at players cards'],
    ...
    def test_is_playable_information_give_self_information_fails(self):
        mock_card = util.create_mock_card(number=2)
        mock_gamestate = util.create_mock_gamestate(hands=[[mock_card, mock_card, mock_card],
                                                           [mock_card, mock_card, mock_card]])
        mock_gamestate.board = util.create_mock_board(clock_tokens=1)
    ...
    def test_is_playable_information_no_clock_tokens(self):
        ...
        mock_gamestate = util.create_mock_gamestate(hands=[['is_playable should', 'not look at players cards'],
                                                           [mock_card, mock_card, mock_card]])
        mock_gamestate.board = util.create_mock_board(clock_tokens=0)
        info_dict = {'player_id': 0, 'information_type': 'number', 'information': 2}
        move = Move(move_type='give_information', player_index=1, information=info_dict)
        assert move.is_playable(mock_gamestate) is False
    ...
    def test_is_playable_information_no_numbers_fails(self):
        mock_card = util.create_mock_card(number=3)
        mock_gamestate = util.create_mock_gamestate(hands=[['is_playable should', 'not look at players cards'],
                                                           [mock_card, mock_card, mock_card]])
        mock_gamestate.board = util.create_mock_board(clock_tokens=1)
        info_dict = {'player_id': 1, 'information_type': 'number', 'information': 2}
    ...
    def test_is_playable_information_no_player_fails(self):
        mock_card = util.create_mock_card(number=1)
        mock_gamestate = util.create_mock_gamestate(hands=[['is_playable should', 'not look at players cards'],
                                                           [mock_card, mock_card, mock_card]])
        mock_gamestate.board = util.create_mock_board(clock_tokens=1)
    ...
    def test_is_playable_information_color_is_wild_fail(self):
        mock_card = util.create_mock_card(color='wild')
        mock_gamestate = util.create_mock_gamestate(hands=[[mock_card, mock_card],
                                                           ['is_playable should', 'not look at players cards']])
        mock_gamestate.board = util.create_mock_board(clock_tokens=1)
    ...
    def test_apply_discard(self):
        ...
        """
        mock_card = util.create_mock_card()
        mock_card_for_discard = util.create_mock_card()
        mock_gamestate = util.create_mock_gamestate(hands=[[mock_card], [mock_card, mock_card_for_discard]])
        mock_gamestate.board = util.create_mock_board()
    ...
    def test_apply_play_blow_fuse(self):
        ...
        mock_card = util.create_mock_card()
        mock_card_for_play = util.create_mock_card(color='red')
        mock_gamestate = util.create_mock_gamestate(hands=[[mock_card], [mock_card_for_play, mock_card]])
    ...
    def test_apply_play_successful_play_incomplete_stack(self):
        ...
        """
        mock_card = util.create_mock_card()
        mock_card_for_play = util.create_mock_card(color='green')
        mock_gamestate = util.create_mock_gamestate(hands=[[mock_card], [mock_card_for_play, mock_card]])
    ...
    def test_apply_give_information_color_once(self):
        ...
        """
        mock_card = util.create_mock_card(color='blue')
        mock_card_other_colors = util.create_mock_card(color='red')
        mock_gamestate = util.create_mock_gamestate(
            hands=[[mock_card_other_colors, mock_card_other_colors, mock_card, mock_card],
                   [mock_card, mock_card_other_colors]])
        mock_gamestate.board = util.create_mock_board(clock_tokens=2)
    ...
    def test_apply_give_information_color_multiple_cards(self):
        ...
        mock_card_other_colors = util.create_mock_card(color='red')
        mock_gamestate = util.create_mock_gamestate(
            hands=[[mock_card_other_colors, mock_card_other_colors, mock_card],
    ...
    def test_apply_give_information_number(self):
        ...
        mock_card = util.create_mock_card(number=2)
        mock_card_other_number = util.create_mock_card(number=4)
        mock_gamestate = util.create_mock_gamestate(
            hands=[[mock_card_other_number, mock_card, mock_card],
    ...
    def test_apply_give_information_number_multiple_cards(self):
        ...
        mock_card_other_number = util.create_mock_card(number=4)
        mock_gamestate = util.create_mock_gamestate(
            hands=[[mock_card_other_number, mock_card, mock_card_other_number],
